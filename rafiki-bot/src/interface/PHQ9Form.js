...
const PHQ9Form = ({ onSubmit, onClose }) => {
  const questions = [
    "kutokupendelea sana au kuwa na furaha kidogo katika kufaya jambo?",
    "kupata mfadhaiko, huzuni au kuishiwa matumaini?",
    "kupata usingizi kwa shida au kulala muda mfupi, au kulala sana?",
    "kujisikia kuchoka au kuishiwa nguvu?",
    "kutokuwa na hamu ya kula au kula kupita kiasi?",
    "kupata hisia mbaya kuhusu wewe mwenyewe au kufikiri kwamba wewe ni wa kushindwa/umeshindwa, au umejiangusha wewe au familia yako chini?",
    "kushindwa kuzingatia vitu, kama vile kusoma gazeti au kuangalia runinga?",
    "kusonga au kuongea taratibu kupita kiasi adi kupelekea watu wengine kuona? au kinyume chake - kutokutulia au kuhangaika sana mpaka kupelekea kuzunguka sehemu moja isivyo kawaida?",
    "mawazo ya kwamba ni bora ukifa au ya kutaka kujidhuru kwa namna yoyote ile?",
 ...
 const handleSubmit = (event) => {
  ...
  const combinedMessage = `alama ya PHQ-9: ${totalScore} | athari kwa maisha ya kila siku: ${impactResponse}`;
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md relative">
      ...
      <h2 className="text-lg font-bold mb-2">Fomu ya Tathmini ya PHQ-9 ya Unyogovu</h2>
