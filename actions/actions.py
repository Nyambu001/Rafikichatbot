...
class ActionExpressStress(Action):
    def name(self) -> Text:
        return "action_express_stress"

    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        feeling = (tracker.get_slot("feeling") or "").strip()
        cause = (tracker.get_slot("cause") or "").strip()

        responses = []

        if not (cause or feeling):
            responses = [
                "Najua hii ni ngumu sana kwako, na ningependa ujue hauko peke yako. Naweza kusaidia kwa namna yoyote?",
                "Nisamehe kama siwezi kuelewa kila kitu unachopitia, lakini ninaweza kusikia uzito wake. Niko hapa kwa ajili yako.",
                "Ninaweza kuhisi uzito wa hayo unayoyapitia. Tafadhali jisikie huru kushiriki chochote utakachotaka.",
                "Pole sana. Kuna kitu chochote kinachoweza kusaidia hata kidogo? Naweza kusikiliza au kushiriki mawazo?",
                "Hii inaweza kuwa wakati mgumu kwako, na ni sawa kuhisi hivyo. Niko hapa kusikiliza na kusaidia.",
                "Unaweza kushiriki zaidi ikiwa uko tayari. Nataka kuelewa zaidi na kuwa hapa kwa ajili yako.",
                "Sio rahisi kupitia hali ngumu bila msaada. Tafadhali kumbuka kuwa hauko peke yako.",
                "Unajali sana kuhusu hili, na inaonyesha. Niko hapa kukusikiliza bila hukumu yoyote.",
                "Najua mambo yanaweza kuwa magumu sana, lakini nataka ujue kuwa unaweza kutegemea msaada wangu.",
                "Siwezi kusema ninaelewa kila kitu, lakini niko hapa kusikiliza na kukuwepo kwa njia yoyote ninayoweza."
            ]
        elif cause and not feeling:
            responses = [
                f"Najua kuwa {cause} inaweza kuwa mzito sana. Tafadhali usisite kushiriki jinsi unavyohisi, niko hapa kwa ajili yako.",
                f"Pole sana kwa changamoto unazopitia kutokana na {cause}. Kumbuka, hauko peke yako, naweza kusikiliza.",
                f"Hii inaweza kuwa changamoto kubwa kwako, haswa kwa sababu ya {cause}. Usihisi mzigo huu peke yako.",
                f"Najua {cause} inaweza kuchosha na kuumiza sana. Unahitaji mtu wa kuzungumza naye? Niko hapa.",
                f"Nisamehe kama siwezi kubeba mzigo huu kwa ajili yako, lakini niko hapa kukusikiliza kuhusu {cause}.",
                f"Unapitia mengi kutokana na {cause}, na hiyo sio rahisi kabisa. Tafadhali jisikie huru kushiriki zaidi.",
                f"Ningependa kujua jinsi {cause} inavyokuathiri zaidi, ikiwa uko tayari kushiriki. Niko hapa kwa ajili yako.",
                f"Najua {cause} inaweza kuwa na athari kubwa kwako. Kumbuka, uko na msaada wangu hapa.",
                f"Hakuna maneno ya kutosha kueleza changamoto ya {cause}, lakini nataka ujue kuwa siko hapa kukuhukumu, bali kukusikiliza.",
                f"Ningependa kukusaidia kupitia hili, hata kama ni kwa kukusikiliza tu. {cause} sio kitu rahisi kupitia peke yako."
            ]
        elif feeling and not cause:
            responses = [
                f"Kuhisi {feeling} kunaweza kuwa mzito sana, na ningependa ujue hauko peke yako katika hili.",
                f"Najua ni vigumu kuhisi {feeling}. Tafadhali jisikie huru kushiriki zaidi, niko hapa kusikiliza.",
                f"Hali kama hii inaweza kuwa ngumu sana. Nataka ujue kuwa uko salama kushiriki chochote unachotaka.",
                f"Kuhisi {feeling} kunaweza kuwa mzito sana. Usijilazimishe kupitia haya peke yako.",
                f"Pole sana unavyohisi {feeling}. Inaweza kuwa gumu sana, lakini hauko peke yako.",
                f"Hili linaweza kuwa wakati mgumu sana kwako. Tafadhali usihisi shinikizo la kujizuia.",
                f"Najua {feeling} si hisia rahisi kuhimili. Tafadhali jisikie huru kuzungumza nami, niko hapa.",
                f"Unajali sana kuhusu hili, na ningeweza kusikia uzito wake. Tafadhali usisite kushiriki zaidi.",
                f"Pole sana kwa hisia hizi unazopitia. Nataka kukusaidia kwa njia yoyote niwezayo.",
                f"Hata bila kujua chanzo, ninajua {feeling} inaweza kuwa gumu. Tafadhali kumbuka kuwa hauko peke yako."
            ]
        else:
            responses = [
                f"Pole sana, najua kuwa {cause} inaweza kusababisha kuhisi {feeling}. Tafadhali kumbuka kuwa hauko peke yako, niko hapa kwa ajili yako.",
                f"Nimeelewa kuwa {cause} imekufanya kuhisi {feeling}. Niko hapa kusikiliza na kusaidia kwa vyovyote ninavyoweza.",
                f"Najua {cause} inaweza kuwa mzito sana, na kuhisi {feeling} ni kawaida kabisa. Tafadhali jisikie huru kushiriki zaidi.",
                f"Ninakuelewa sana, {cause} inaweza kuwa ngumu, na kuhisi {feeling} ni hali ya kawaida. Tafadhali usihisi mzigo huu peke yako.",
                f"Pole sana kwa yale unayopitia. {cause} inaweza kuwa changamoto kubwa, na ninaelewa kwa nini unahisi {feeling}.",
                f"Unapitia mengi kwa sababu ya {cause}, na hiyo inaweza kusababisha kuhisi {feeling}. Tafadhali kumbuka kuwa uko salama kushiriki.",
                f"Nataka ujue kuwa unajaliwa, {cause} inaweza kuwa ngumu sana, na kuhisi {feeling} ni hali halali kabisa.",
                f"Najua {cause} inaweza kuwa na athari kubwa kwako, na kuhisi {feeling} ni jambo la kawaida kabisa katika hali hii.",
                f"Pole sana, najua kuwa {cause} imekufanya kuhisi {feeling}. Tafadhali usisite kushiriki mawazo yako zaidi, niko hapa kwa ajili yako.",
                f"Hili linaweza kuwa jambo gumu sana kwako, {cause} inaweza kuwa na uzito mkubwa, na kuhisi {feeling} ni kitu ninachoweza kuelewa."
            ]
